...
- **Phase 66.1 - The Physical Siege Engine**: Bridges Geography, Combat, Economics, and Sovereignty. Implemented `SiegeSystem` (`internal/systems/siege.go`) and the `SiegeMarker` DOD component. When a Village is outnumbered by hostile NPCs belonging to an attacking `CountryID` during an active war, the system applies the `SiegeMarker`. This organically spikes local `MarketComponent.FoodPrice` and drains `LoyaltyComponent.Value`. This mechanically simulates a siege by choking the economy and forcing the `VassalRebellionSystem` or `LaborCrisisSystem` to trigger collapse naturally.
