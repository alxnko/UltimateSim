...
- **Phase 69.1 - Parasitic Symbiosis**: Bridges Biology (Vitals), Justice (Witch Hunt), and Geography. Implemented `ParasiteComponent` and `ParasiteSystem`. The system pre-caches healthy victims and iterates over hungry parasites. They pathfind to healthy NPCs and drain their blood to restore their own food needs, bypassing the agrarian economy. The act generates a deep `-100` grudge hook from the victim to the parasite via `SparseHookGraph` and explicitly tags the parasite with the `TraitEsoteric` bitmask, perfectly linking the biological vampiric act into the state-sponsored `WitchHuntEngine` (Phase 49) and `BloodFeudSystem` (Phase 23).
