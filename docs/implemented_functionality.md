...
- **Phase 66 - The Physical Siege Engine**: Bridges Phase 29 (Geopolitical Resource Wars) and Phase 13 (Macroeconomics). Implemented `SiegeSystem` tracking active wars and dynamically counting hostile and defending NPCs surrounding a `Village`. If a village is outnumbered, it receives a `SiegeMarker`. While besieged, the village's `MarketComponent.FoodPrice` spikes drastically and `LoyaltyComponent` drains, natively bridging military blockade into systemic famine and subsequent vassal rebellions/refugee crises without relying on direct `CombatSystem` logic.
