...
- **Date**: [Current]
- **System**: `MedicalSystem` (`internal/systems/medical.go`)
- **Evolutionary Goal**: Adds systemic depth and cross-pillar connectivity (Biology -> Economy -> Justice). NPCs previously lacked a localized, physical method to recover from `CombatMarker` or `BiologicalSabotage` damage. The Medical Engine introduces `JobDoctor` NPCs who physically navigate the map to heal injured citizens.
- **The Butterfly Effect**: Healthcare is not an abstract background state. It requires physical proximity and economic transaction. If a desperately injured NPC is healed but cannot afford the `Needs.Wealth` cost, the Doctor enforces a `-50` negative hook via the `SparseHookGraph`. This completely dynamic interaction guarantees that systemic poverty loops recursively drive `BloodFeudSystem` and `MercenarySystem` violence, exactly mirroring the vision for total macroscopic simulation generated by micro-scale agency.
