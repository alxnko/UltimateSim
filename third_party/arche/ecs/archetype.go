...
type archetypeData struct {
	...
	barrier      []bool          // PATCH: per column, whether the type contains Go pointers (needs write barriers on copy).
}
...
func (a *archetype) Init(node *archNode, data *archetypeData, index int32, forStorage bool, layouts uint8, relation Entity) {
	...
	// PATCH: precompute which columns hold pointerful types. Copies of those
	// columns must go through reflect (write barriers); raw byte copies break
	// the GC hybrid write barrier and cause "found pointer to free object".
	a.barrier = make([]bool, len(node.Ids))
	for i, tp := range node.Types {
		a.barrier[i] = typeNeedsBarrier(tp)
	}
...
func (a *archetype) Remove(index uint32) bool {
	...
	if index != old {
		for i, id := range a.node.Ids {
			...
			// PATCH: barrier-aware swap-remove copy for pointerful columns.
			if a.barrier[i] {
				typedCopy(a.node.Types[i], src, dst)
			} else {
				a.copy(src, dst, size)
			}
		}
...
func (a *archetype) Zero(index uint32, id ID) {
	...
	dst := unsafe.Add(lay.pointer, index*size)
	// PATCH: barrier-aware zeroing (deletion barrier) for pointerful columns.
	if tp, ok := a.columnBarrier(id); ok {
		typedZero(tp, dst)
		return
	}
...
func (a *archetype) ZeroRange(start, len uint32, id ID) {
	...
	// PATCH: barrier-aware zeroing (deletion barrier) for pointerful columns.
	tp, needsBarrier := a.columnBarrier(id)
	var i uint32
	for i = 0; i < len; i++ {
...
func (a *archetype) SetPointer(index uint32, id ID, comp unsafe.Pointer) unsafe.Pointer {
	...
	// PATCH: barrier-aware copy for pointerful columns (entity moves between
	// archetypes flow through here).
	if tp, ok := a.columnBarrier(id); ok {
		typedCopy(tp, comp, dst)
		return dst
	}
...
func (a *archetype) CopyFrom(other *archetype, id ID, startIndex uint32) {
	...
	// PATCH: barrier-aware bulk copy for pointerful columns.
	if tp, ok := a.columnBarrier(id); ok {
		var i uint32
		for i = 0; i < other.len; i++ {
			typedCopy(tp, unsafe.Add(src, i*size), unsafe.Add(dst, i*size))
		}
		return
	}
...
// PATCH: typeNeedsBarrier reports whether tp contains Go pointers,
// in which case copies must be typed so GC write barriers run.
func typeNeedsBarrier(tp reflect.Type) bool {
	switch tp.Kind() {
	case reflect.Ptr, reflect.UnsafePointer, reflect.Slice, reflect.Map,
		...
		return true
	case reflect.Array:
		return tp.Len() > 0 && typeNeedsBarrier(tp.Elem())
	case reflect.Struct:
		for i := 0; i < tp.NumField(); i++ {
			if typeNeedsBarrier(tp.Field(i).Type) {
				return true
			}
		}
	}
...
// PATCH: typedCopy copies one component item through reflect so that GC
// write barriers see pointer fields. Raw byte copies of pointerful
// component memory violate the hybrid write barrier during concurrent
// marking; the pointee can be swept free while still referenced, which the
// GC later reports as "found pointer to free object" (zombie object).
func typedCopy(tp reflect.Type, src, dst unsafe.Pointer) {
	reflect.NewAt(tp, dst).Elem().Set(reflect.NewAt(tp, src).Elem())
}

// PATCH: typedZero zeroes one component item through reflect so the
// deletion write barrier shades the old pointees.
func typedZero(tp reflect.Type, dst unsafe.Pointer) {
	reflect.NewAt(tp, dst).Elem().SetZero()
}

// PATCH: columnBarrier returns the column's type and whether it needs
// barrier-aware copies. ok is false for columns not in this archetype.
func (a *archetype) columnBarrier(id ID) (reflect.Type, bool) {
	idx, ok := a.indices.Get(id.id)
	if !ok || !a.barrier[idx] {
		return nil, false
	}
	return a.node.Types[idx], true
}
