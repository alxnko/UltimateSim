...
func SaveWorld(tm *TickManager, mapGrid *MapGrid, seedVal byte, db *sql.DB) error {
	...
	tables := []string{"entities", "identity", "position", "needs", "affiliation", "tags", "storage", "velocity", "job", "memory", "beliefs", "genome", "vitals", "population", "desperation", "secrets", "equipment", "sanity", "treasury", "market", "legitimacy", "loyalty", "jurisdiction", "structure", "construction_site", "demographics", "workbench", "culture", "extra_tags"}
	...
	for query.Next() {
		...
		if world.Has(ent, structID) {
			...
			if _, err := stmtStruct.Exec(uid, s.StructureType, s.Integrity, 0, 0); err != nil {
				query.Close()
				return err
			}
		}
		...
		if world.Has(ent, constrID) {
			...
			if _, err := stmtConstr.Exec(uid, c.WoodRequired, c.WoodGathered, c.StoneRequired, c.StoneGathered, c.Progress, c.MaxProgress, c.BuilderID, 0); err != nil {
				query.Close()
				return err
			}
		}
...
func LoadWorld(tm *TickManager, db *sql.DB) error {
	...
	rowsStruct, err := db.Query("SELECT uid, structure_type, integrity FROM structure")
	if err != nil {
		return err
	}
	...
	for rowsStruct.Next() {
		var u uint64
		var s structData
		if err := rowsStruct.Scan(&u, &s.sType, &s.integrity); err != nil {
			return err
		}
		structMap[u] = s
	}
	...
	rowsConstr, err := db.Query("SELECT uid, wood_req, wood_gathered, stone_req, stone_gathered, progress, max_progress, builder_id FROM construction_site")
	if err != nil {
		return err
	}
	...
	for rowsConstr.Next() {
		var u uint64
		var c constrData
		if err := rowsConstr.Scan(&u, &c.woodReq, &c.woodGot, &c.stoneReq, &c.stoneGot, &c.progress, &c.maxProgress, &c.builderID); err != nil {
			return err
		}
		constrMap[u] = c
	}
