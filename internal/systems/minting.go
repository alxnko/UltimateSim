...
func (s *MintingSystem) Update() {
	...
	// 1. Iterate over all Capital Villages with Storage, Position and CountryComponent
	query := s.world.Query(filter.All(s.villageID, s.capitalID, s.countryID, s.storageID, s.affilID, s.posID))
	for query.Next() {
		...
		country := (*components.CountryComponent)(query.Get(s.countryID))

		// Phase 15.4: Organic Inflation via Debasement
		// Reduce iron cost based on country's debasement rate.
		// Debasement is a float32 typically from 0.0 to 0.99
		ironCost := uint32(100.0 * (1.0 - country.Debasement))
		if ironCost < 1 {
			ironCost = 1
		}

		// If the capital has sufficient raw material
		if storage.Iron >= ironCost {
			// Deduct the cost deterministically
			storage.Iron -= ironCost
	...
	for _, spawn := range s.toSpawn {
		...
		curr.Debasement = spawn.debasement
	}
}
