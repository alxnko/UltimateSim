...
// Phase 03.2: The Genesis Spawner
// Run once at Tick 0. Queries MapGrid for walkable/habitable tiles.
// Uses GlobalRNG to select 100 starting locations.

type FamilySpawnerSystem struct {
	mapGrid    *engine.MapGrid
	hasSpawned bool
	nextID     uint64
}

// NewFamilySpawnerSystem creates a new FamilySpawnerSystem.
func NewFamilySpawnerSystem(world *ecs.World, mapGrid *engine.MapGrid) *FamilySpawnerSystem {
	return &FamilySpawnerSystem{
		mapGrid:    mapGrid,
		hasSpawned: false,
...
func (s *FamilySpawnerSystem) Update(world *ecs.World) {
	if s.hasSpawned {
	...
	for i := 0; i < spawnCount; i++ {
		...
		entity := world.NewEntity(posID, velID, idID, genID, legID, needsID, pathID, fcID, slID)
		...
		id.ID = s.nextID
		s.nextID++
		id.Name = fmt.Sprintf("Cluster-%d", id.ID)
